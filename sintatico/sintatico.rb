...
=end
  ...
  def log_erro(token,code_error)
    ...
    puts "Código de erro #{code_error} linha #{@lexer.linha_atual} - encontrado: [#{valor_token(@token)}] - motivo: #{ erros[code_error]}"
    exit
  end
end
